"""Checks that the fields.json file does not have overlapping bounding boxes. See forms.md."""
...
@dataclass
class RectAndField:
    """Bounding rect paired with its source field and rect type (label or entry)."""
...
def get_bounding_box_messages(fields_json_stream) -> list[str]:
    """Return a list of validation messages for Claude to read."""
    messages = []
    fields = json.load(fields_json_stream)
    messages.append(f"Read {len(fields['form_fields'])} fields")

    def rects_intersect(r1, r2):
        """Return True if rectangle r1 and r2 overlap (sharing only an edge is not an overlap)."""
            ...
            if same_page and rects_intersect(ri.rect, rj.rect):
                ...
                if ri.field is rj.field:
                    ...
                    messages.append(
                        ...
                        f" for `{ri.field['description']}` ({ri.rect})"
                        f" and {rj.rect_type} bounding box"
