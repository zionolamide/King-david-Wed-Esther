...
export default function CardPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  useEffect(() => {
    async function fetchCard() {
      try {
        const res = await fetch(`/api/access-card?code=${code}`);
        const json = await res.json();
        if (json.ok) {
          setData({ title: json.title, fullName: json.fullName, entryCode: json.entryCode });
        } else {
          setError(json.message || "Invalid access code");
        }
      } catch {
  ...
  async function downloadCard() {
    ...
    try {
      ...
      link.download = `KDE2026-${data?.entryCode || "access-card"}.png`;
      link.href = dataUrl;
      link.click();
    } catch {
      window.print();
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-ivory">
        <div className="flex flex-col items-center gap-3">
          <Heart className="h-8 w-8 animate-pulse text-wine" />
          <p className="text-sm text-ink/60">Loading your access card...</p>
        </div>
      </main>
    );
  }
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-ivory p-6">
        <div className="max-w-sm text-center">
          ...
          <p className="mt-3 text-sm text-ink/60">{error || "Card not found"}</p>
        </div>
      </main>
    );
  }
