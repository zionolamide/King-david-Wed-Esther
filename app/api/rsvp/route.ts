...
export async function POST(request: Request) {
  ...
  if (!isSimulated) {
    ...
    const { error } = await supabase.from("rsvp_submissions").insert({
      ...
      email,
      phone: phone || null,
      note: note ? JSON.stringify({ wish: note }) : null,
  ...
  if (emailUser && emailPassword) {
    ...
    // Build the email access card HTML
    const emailCardHtml = `
    ...
    async function sendMailWithRetries(transport: any, options: any, attempts = 3) {
      for (let i = 0; i < attempts; i += 1) {
        try {
          ...
          else console.error("Failed to send email after retries:", err);
        }
      }
      return false;
    }
