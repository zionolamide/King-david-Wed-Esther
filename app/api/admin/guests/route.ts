...
export async function PATCH(request: Request) {
  ...
  // Approve guest — sets approved:true and sends access card email
  if ((body as any).approve_guest) {
    ...
    const { data: guest } = await supabase.from("rsvp_submissions").select("*").eq("id", body.id).maybeSingle();
    if (!guest) return NextResponse.json({ ok: false, message: "Guest not found" }, { status: 404 });
